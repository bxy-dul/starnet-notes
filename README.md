# 《Variational Inference for Deblending Crowded Starfields》中文论文笔记（最终 GitHub 上传版）
...
图 10 展示了颜色-星等图（CMD）。这类图在天文学里非常重要，因为它反映的是天体群体统计性质。
...
这篇论文最有价值的地方，不只是提出了一个叫 StarNet 的模型，而是给出了一个很完整的思路：

- 从物理成像过程出发建模；
- 用 tile 化和 padding 解决工程上的复杂性；
- 用 forward KL 把复杂推断问题转成可训练的监督学习；
- 最终在真实密集星场上验证效果。

对“密集星场处理”项目中的源探测模块来说，它最重要的启发可以概括成一句话：

> **在密集星场里，源探测不是找峰值，而是对重叠亮斑做最合理的多源解释。**

如果后续要继续往实现方向走，我认为最合适的起步路线是：

**先做模拟器 → 再做 tile 级源数分类 + 位置/亮度回归 → 再做整图拼接与评估。**

---

## 附：建议在 GitHub 仓库中保留的目录结构
